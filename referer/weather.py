...
def send_parse_urls(start_urls):
    ...
    for start_url in start_urls:
        ...
        pattern = re.compile('<td width="83" height="23".*?<a href="(.*?)".*?target="_blank">(.*?)</a>', re.S)
...
def get_start_urls():
    start_urls = [
        ...
        "http://www.weather.com.cn/textFC/gat.shtml",  # 港澳台  # 按关键词搜索就不怕，比如，北京朝阳区， 北京市朝阳区
    ]
    return start_urls
...
def main():
    ...
    for city, link in data.items():
        url1 = link  # 7天天气中国天气网
        url2 = link.replace("weather/", "weather15d/")  # 8-15天天气中国天气网
        ...
        write_to_csv(city + 'weather14.csv', data14, 14)  # 未来14天的天气
        # write_to_csv('weather14/'+city+'weather14.csv',data14,14)
        write_to_csv(city + 'weather1.csv', data1, 1)  # # 今天的天气

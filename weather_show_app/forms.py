...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={'class': 'mdui-textfield-input', 'placeholder': "密码"}))  # 密码啊


class RegistrationForm(forms.ModelForm):  # 这个是注册表单的,表单继承model就是modelform
    username = forms.CharField(label="用户名",
                               widget=forms.TextInput(attrs={'class': 'mdui-textfield-input', 'placeholder': "用户名"}))
    ...
    email = forms.CharField(label='邮箱', widget=forms.EmailInput(
        attrs={'type': "email", 'class': 'mdui-textfield-input', 'placeholder': "密码"})
